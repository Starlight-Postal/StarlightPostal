...
public class wind_fill : wind_structure
{
    ...
    public wind wind_field;

    void Start()
    {
        trans = gameObject.GetComponent<Transform>();
        gameObject.GetComponent<SpriteRenderer>().enabled = false;
        wind_field = GameObject.Find("wind").GetComponent<wind>();
    }

    public override Vector2 getWind(float x, float y)
    {
        Vector2 wind = new Vector2(0, 0);
        //Vector2 w = wind_field.getWind(x, y);
        if (wind_field.w.magnitude == 0)
        {
